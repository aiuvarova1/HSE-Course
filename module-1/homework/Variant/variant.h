...
public:
    ...
    static void Set(T&& value, std::in_place_index_t<0>,
                    UnionByType<TypeIdx, Head, Tail...>& union_by_type) {
    ...
    template <size_t Idx, std::size_t TypeIdx, typename T, typename Head, typename... Tail>
    static void Set(T&& value, std::in_place_index_t<Idx>,
                    UnionByType<TypeIdx, Head, Tail...>& union_by_type) {
...
struct Search {
    ...
    constexpr static bool kIsConvertibleType[sizeof...(Types)] = {
        std::is_convertible<T, Types>::value...};
    constexpr static std::size_t kExactIdx = PositionOf(kIsExactType);
    constexpr static std::size_t kConvertibleIdx =
        kExactIdx != -1 ? kExactIdx : PositionOf(kIsConvertibleType);
};
...
constexpr const variant_alternative_t<I, Variant<Types...>>& Get(Variant<Types...>& v) {
    return VariantUtils::Get<typename TypeAt<I, TypeList<Types...>>::type>(
        std::forward<Variant<Types...>>(v));
}
